...
选择出数组中的最小元素，将它与数组的第一个元素交换位置。再从剩下的元素中选择出最小的元素，将它与数组的第二个元素交换位置。如此往复，直到将整个数组排序。
...
- 最好的情况下需要 N-1 次比较和 0 次交换，即初始数组已经有序了。
...
希尔排序的运行时间达不到平方级别，使用**递增序列 1, 4, 13, 40, ...（h = 3 * h + 1）**  的希尔排序所需要的比较次数不会超过 N 的若干倍乘于递增序列的长度。**希尔排序的代码量很小，且不需要额外的内存空间**，所以有经验的程序员有时会选用希尔排序处理中等大小的数组。除了对于很大的 N ，后面介绍的**高级排序算法只会比希尔排序快两倍左右**。
...
将数组中两个已经排序的部分归并成一个，将涉及排序的所有元素复制到一个辅助数组中，再把归并的结果放回原数组中。
...
**分治思想**：将原始问题划分或者归结为规模较小的子问题，递归或迭代求解每个子问题（独立求解），从而得到原问题的解。
...
public class Up2DownMergeSort<T extends Comparable<T>> extends MergeSort<T> {
    ...
    @Override
    public void sort(T[] nums) {
        aux = (T[]) new Comparable[nums.length];//一次性分配辅助数组的空间
...
对于长度为 N 的任意数组，自顶向下的归并排序需要 1/2NlgN ~ NlgN 次比较，最多访问数组 6NlgN 次，即归并排序所需时间和 NlgN 成正比。

### 改进

- **对小规模子数组采用插入排序**：因为递归会使小规模问题中方法的调用过于频繁，因此在小数组上（如长度小于 15），插入排序非常简单，很可能比归并排序更快。
- **归并前测试数组是否有序**：如果 `a[mid] <= a[mid + 1]` ，那么我们就认为数组已经是有序的，可以跳过当前的 merge() 方法，且不会影响到整ui体的递归调用。
- **不将元素复制到辅助数组：**将数据从输入数组排序到辅助数组，或将数据从辅助数组排序到输入数组，这需要在递归调用的每个层次交换输入数组和辅助数组的角色。
...
当数组长度为 2 的幂时，自底向上和自顶向上的归并排序所用的比较次数和数组访问次数是一样的，只不过次序会有所不同。

自底向上的归并排序比较适合**用链表组织的数据**，只需重新组织链表连接，就能将链表原地排序。

**归并排序在最坏情况下的比较次数和任意基于比较的排序算法所需的比较次数都是 ~NlgN。**
