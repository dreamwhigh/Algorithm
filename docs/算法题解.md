...
两次嵌套遍历，，对于每组 i 和 j（其中 j > i），找出 max(prices[j] − prices[i])
...
- 先查找一个最小谷值 minprice，然后继续找最大峰值对应的利润 maxprice
- 如果出现了更小的谷值，则更换最小谷值
- 再找最大峰值对应的利润 prices[i] - minprice，若比当前最大利润大，则更换最大利润值
- 时间复杂度 O(n)，空间复杂度 O(1)
...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        int minprice = Integer.MAX_VALUE;//最小谷值
        for(int i = 0; i < prices.length; i++){
        	if(minprice > prices[i]) {
        		minprice = prices[i];//更换最小谷值
        	}else if(prices[i] - minprice > maxprofit) {
        		maxprofit = prices[i] - minprice;//更换最大利润
        	}    
        }
        return maxprofit;        
    }
...
[贪心算法 图解](<https://leetcode-cn.com/problems/best-time-to-buy-and-sell-stock-ii/solution/best-time-to-buy-and-sell-stock-ii-zhuan-hua-fa-ji/>)

#### 解题思路

- profit 为第 i-1 日买入，第 i 日卖出的利润，`profit = prices[i] - prices[i - 1]`
- 若利润大于 0 ，直接加入总利润 maxprofit 中；如利润为 0 或为负时，直接跳过，不做交易
...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        for(int i = 1; i < prices.length; i++){
        	profit = prices[i] - prices[i - 1];
        	if(profit >= 0){//如果利润大于等于0，则加入上一次的利润maxprofit[j]中
                maxprofit[j] += profit;
            }else{//如果利润小于0，则重新计算下一轮的利润maxprofit[++j]
                maxprofit[++j] = 0;
            }        		
        }
        Arrays.sort(maxprofit);//利润值数组排序
        //返回最大的两次利润值之和
        return maxprofit[maxprofit.length - 1] + maxprofit[maxprofit.length - 2];        
    }
}
```
...
[股票通用状态机方法](<https://leetcode-cn.com/problems/best-time-to-buy-and-sell-stock-iii/solution/yi-ge-tong-yong-fang-fa-tuan-mie-6-dao-gu-piao-wen/>)
...
- 设定一个标志位 `flag`，用来判断当前层的遍历方向，对于`flag == -1`，遍历方向为从右至左，将 subres 子 list 反转后加入 res list 中
...
TP-Link 2019 秋招笔试题 [nowcoder](<https://www.nowcoder.com/practice/6ab1d9a29e88450685099d45c9e31e46?tpId=13&tqId=11189&tPage=1&rp=1&ru=/ta/coding-interviews&qru=/ta/coding-interviews/question-ranking>)
...
不可能会是一个“X”型交叉的情况，因为公共结点只有一个 next 指针，其后不会出现两个自结点。
...
1. 求出两个链表的长度差 len，指向较长链表的指针先向后移动 len 个结点，然后两个指针再一起同步向后移动，这样保证了两个指针同时到达链表尾部。
...
2. cur1 先扫描链表 pHead1，再扫描 pHead2；cur2 先扫描链表 pHead2，再扫描 pHead1；这可以保证在两个指针扫描的总结点数相同，即同时到达后一个链表的尾结点，避免了两次遍历过程求链表长度。
...
3. 利用 HashSet，将第一个链表中的结点存入 set，遍历第二个链表，遍历到的第一个存在于 set 中的结点即为公共子结点。
...
public class Solution {
    public ListNode FindFirstCommonNode(ListNode pHead1, ListNode pHead2) {
        ...
        //将链表 pHead1 中的结点全部存入 HashSet
        while(cur1 != null){
            set.add(cur1);
            cur1 = cur1.next;
        }
        //遍历链表 pHead2，看 HashSet 中是否有这个结点
        while(cur2 != null){
            //该结点存在于 set 中，即为第一个公共结点
...
TP-Link 2019 秋招笔试题[nowcoder]( https://www.nowcoder.com/questionTerminal/cf209ca9ac994015b8caf5bf2cae5c98>)
...
```
input： 8 //总人数
186 186 150 200 160 130 190 220 //身高
output：4
```
...
选定第 i 个同学为最高点，分别求出 i 左边的最长递增子序列长度 left[i]，i 右边的最长递减子序列长度 right[i]，剩余同学的数量为 left[k] + right[k] - 1，减去重复计算的第 i 个同学。选取  left[k] + right[k] - 1 的最大值，对应的就是需要出列学生的最小值 cnt - ( left[k] + right[k] - 1)。
...
public class singQueue {
	public static void main(String[] args) {
		...
		while(in.hasNextInt()) {			
			int cnt = in.nextInt();
			System.out.println("输入每个人的身高");
			int[] height = new int[cnt];
			for(int i = 0; i < cnt; i++) {
				height[i] = in.nextInt();
			}
			System.out.println("输入完成");
			System.out.println(creatSingQueue(height, cnt));
		}
		in.close();
		
	}
	//left[k]表示从左边起到第k个人结束的最长上升队列的人数
	//righ[k]表示从右边起到第k个人结束的最长上升队列的人数
	private static int creatSingQueue(int[] height, int cnt) {
		...
		for(int k = 1; k < cnt; k++) {
			...
			for(int i = 0; i < k; i++) {
				if(height[i] < height[k])
					left[k] = Math.max(left[i] + 1, left[k]);
			}
		}
		for(int k = cnt - 2; k >= 0; k--) {
			...
			for(int i = cnt - 1; i > k; i--) {
				if(height[i] < height[k])
					right[k] = Math.max(right[i] + 1, right[k]);
			}
		}
		int res = 0;
		for(int k = 0; k < cnt; k++) {
			int temp = left[k] + right[k] - 1;//减去重复计算的第k个人
			if( temp > res)
				res = temp;			
		}
		return cnt - res;		
